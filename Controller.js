...
function Controller(HTMLgamepad) {
    ...
    function initControllerMapping() {
        ...
        let has_layout = this.constructor.layouts.has(this.name);
        let has_inexact_layout = this.constructor.layouts.has_inexact(this.name)
        if (has_layout) {
            unknownLayout = false;
            layout = this.constructor.layouts.get(this.name);
        } else if (has_inexact_layout) {
            unknownLayout = false;
            layout = this.constructor.layouts.get_inexact(this.name);
        ...
        // console.log("using layout", this.name, layout)
...
var GC_Layouts = {
    ...
    has_inexact: function has_inexact(name) {
        name = name.toLowerCase();
        // console.log(name, Controller.layouts.list)
        for (let layout of Object.values(Controller.layouts.list)) {
            if ("inexact_match" in layout && name.indexOf(layout.inexact_match.toLowerCase()) >= 0) {
                return true;
            }
        }
        return false;
